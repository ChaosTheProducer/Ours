...
"""
Pre-Train script for TTT task Rotation Predictor
"""
...
def main(args):
    ...
    for epoch in trange(args.max_epoch):
        """
        Training Auxiliary Task, Rotation Predictor
        """
        print("Start training Auxiliary task! (Rotation Predictor)")
        loss_aux_meter = utils.AverageMeter()

        for idx, data in enumerate(train_loader):

            feature_model.train()  # Feature Extract
            rotation_predictor.train()  # Set to train mode
            ...
            # 预测旋转角度
            predicted_rotation = rotation_predictor(features)  #这里是4个角度的逻辑分数
        ...
        print("Epoch {}, loss_aux(Rotation Predictor) {:.5f}\n".format(epoch, loss_aux_meter.avg), flush=True)
